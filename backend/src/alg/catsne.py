...
def catsne_g(X_lds, sigma_ijt, nit, eei, eef, n_eps):
    ...
    s_ijt, log_s_ijt, idsld = catsne_ld_sim(
        ds_ld=scipy.spatial.distance.squareform(X=scipy.spatial.distance.pdist(X=X_lds, metric='sqeuclidean'),
...
def catsne_mgd(ds_hd, labels, theta, n_eps, eei, eef, X_lds, ftol, N, dim_lds, mom_t, nit_max, gtol, mom_init, mom_fin):
    ...
    stepsize, delta_bar = epsilon * np.ones(shape=(N, dim_lds), dtype=np.float64), np.zeros(shape=(N, dim_lds),
                                                                                            dtype=np.float64)
    # Update of X_lds
    ...
    while (nit <= eei) or (nit <= mom_t) or ((nit < nit_max) and (gradn > gtol) and (rel_obj_diff > ftol)):
        ...
        X_lds, up_X_lds = mgd_step(X=X_lds, up_X=up_X_lds, nit=nit, mom_t=mom_t, mom_init=mom_init, mom_fin=mom_fin,
...
def catsne(X_hds, labels, theta=0.9, init='ran', dim_lds=2, nit_max=1000, rand_state=None, hd_metric='euclidean',
           D_hd_metric=None, gtol=10.0 ** (-5.0), ftol=10.0 ** (-8.0), eef=4, eei=100, mom_init=0.5, mom_fin=0.8,
           mom_t=250):
    """
    ...
    if (theta < 0.5) or (theta >= 1):
        raise ValueError(
            "Error in function catsne: theta={theta} whereas it must range in [0.5,1[.".format(theta=theta))
    # Checking rand_state
    ...
    if isinstance(init, str):
        if init == 'ran':
            ...
            if isinstance(hd_metric, str) and (hd_metric == "precomputed"):
                raise ValueError(
                    "Error in function catsne: init cannot be set to 'pca' when hd_metric is set to 'precomputed'.")
            X_lds = sklearn.decomposition.PCA(n_components=dim_lds, copy=True, random_state=rand_state).fit_transform(
                X_hds)
        else:
            raise ValueError(
                "Error in function catsne: init={init} whereas it must either be equal to 'ran' or to 'pca'.".format(
                    init=init))
    else:
        # init must be a 2-D numpy array with N rows and dim_lds columns
        if init.ndim != 2:
            raise ValueError(
                "Error in function catsne: init.ndim={v} whereas init must be a 2-D numpy array.".format(v=init.ndim))
        if init.shape[0] != N:
            raise ValueError(
                "Error in function catsne: init.shape[0]={v} whereas it must equal N={N}.".format(v=init.shape[0], N=N))
        if init.shape[1] != dim_lds:
            raise ValueError(
                "Error in function catsne: init.shape[1]={v} whereas it must equal dim_lds={dim_lds}.".format(
                    v=init.shape[1], dim_lds=dim_lds))
        X_lds = init
    ...
    if isinstance(hd_metric, str):
        if hd_metric == "precomputed":
            ...
            if D_hd_metric is None:
                D_hd_metric = {}
            ds_hd = scipy.spatial.distance.squareform(
                X=scipy.spatial.distance.pdist(X=X_hds, metric=hd_metric, **D_hd_metric), force='tomatrix') ** 2.0
    else:
    ...
    return catsne_mgd(ds_hd=ds_hd, labels=labels, theta=theta, n_eps=n_eps, eei=eei, eef=eef, X_lds=X_lds, ftol=ftol,
                      N=N, dim_lds=dim_lds, mom_t=mom_t, nit_max=nit_max, gtol=gtol, mom_init=mom_init, mom_fin=mom_fin)
...
def viz_digits(X, lab, tit='', cmap='gnuplot2', stit=30, slab=15, wlab='bold', max_ti=None):
    ...
    if len(X.shape) != 2:
        raise ValueError(
            "Error in function viz_digits: X must be a numpy array with shape (N, 2), where N is the number of data points.")
    if X.shape[1] != 2:
    ...
    for i in range(N):
        ax.text(x=X[i, 0], y=X[i, 1], s=str(lab[i]), fontsize=slab - 10.0 * (1.0 - max_ti[i]),
                color=cmap_obj(normalizer(lab[i])), fontdict={'weight': wlab, 'size': slab - 10.0 * (1.0 - max_ti[i])},
...
if __name__ == '__main__':
    ...
    d_hd = scipy.spatial.distance.squareform(X=scipy.spatial.distance.pdist(X=X_hds, metric=hd_metric),
                                             force='tomatrix')

    # For each theta
    for theta in Ltheta:
        print("Applying cat-SNE with threshold theta = {theta}".format(theta=theta))
        X_lds, max_ti = catsne(X_hds=X_hds, labels=labels, theta=theta, init=init, dim_lds=dim_lds, nit_max=nit_max,
                               rand_state=np.random.RandomState(0), hd_metric=hd_metric)
        # Computing the pairwise distances in the LDS for the quality assessment.
        d_ld = scipy.spatial.distance.squareform(X=scipy.spatial.distance.pdist(X=X_lds, metric='euclidean'),
                                                 force='tomatrix')
